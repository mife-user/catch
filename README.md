# 🎯 Catch - 本地文件夹内容搜索工具

一个用 Go 语言开发的轻量级本地文件夹内容搜索命令行工具，支持交互式界面和协程池并发搜索。

## ✨ 功能特性

### 🔍 搜索功能
- **关键字搜索** - 支持按关键字搜索文件内容和文件名
- **正则表达式搜索** - 支持正则表达式高级匹配
- **多关键字搜索** - 支持 AND/OR 逻辑组合搜索
- **递归/非递归搜索** - 可选择是否搜索子目录
- **上下文展示** - 显示匹配行前后 N 行代码上下文
- **忽略文件支持** - 自动读取 `.catchignore` 和 `.gitignore`

### ⚡ 性能优化
- **协程池并发** - 固定数量协程处理文件搜索，避免资源占用过高
- **并发目录遍历** - 目录遍历也使用并发，大幅提升扫描速度（3-5倍）
- **进度条动画** - 实时显示搜索进度、速度、预计剩余时间
- **分页展示** - 大量结果时分页查看

### 🎨 用户体验
- **交互式界面** - 类似 Claude CLI 风格，支持数字选择功能
- **高亮展示** - 搜索结果关键字高亮显示
- **搜索历史** - 记录最近搜索历史，支持快速重复搜索
- **配置文件** - 支持 `.catchrc` 自定义默认参数
- **结果导出** - 支持导出为 JSON/CSV/TXT 格式

### 🌍 跨平台支持
- Windows / macOS / Linux
- 自动构建和发布，开箱即用

## 📥 安装

### 方式 1: 下载预编译二进制文件（推荐）

从 [Releases](https://github.com/yourusername/catch/releases) 页面下载对应平台的可执行文件：

#### Windows
```powershell
# 下载 catch-windows-amd64.exe
# 重命名为 catch.exe 并添加到 PATH
move catch-windows-amd64.exe catch.exe
# 或者添加到 PATH
setx PATH "%PATH%;C:\path\to\catch"
```

#### macOS
```bash
# 下载并添加执行权限
chmod +x catch-darwin-amd64
sudo mv catch-darwin-amd64 /usr/local/bin/catch
```

#### Linux
```bash
# 下载并添加执行权限
chmod +x catch-linux-amd64
sudo mv catch-linux-amd64 /usr/local/bin/catch
```

### 方式 2: 从源码编译

```bash
# 克隆仓库
git clone https://github.com/yourusername/catch.git
cd catch

# 编译
go build -o catch ./cmd/catch

# 移动到 PATH
sudo mv catch /usr/local/bin/
```

## 🚀 运行

### 直接运行

```bash
# 进入交互模式
./catch

# 命令行搜索
./catch search <关键字> [选项]
```

### 示例

```bash
# 搜索关键字 "hello"
./catch search hello

# 递归搜索
./catch search hello -r

# 指定文件类型
./catch search func -t .go

# 指定搜索路径
./catch search config -p ./src -r
```

## ➕ 添加到环境变量

### 自动添加

运行以下命令：

```bash
# Linux/macOS
./catch add-path

# Windows (需要管理员权限)
catch.exe add-path
```

### 手动添加

#### Windows

1. 右键点击"此电脑" -> "属性"
2. 点击"高级系统设置"
3. 点击"环境变量"
4. 在"系统变量"中找到"Path"
5. 点击"编辑" -> "新建"
6. 添加 catch 可执行文件所在路径

#### macOS/Linux

在 `~/.bashrc` 或 `~/.zshrc` 中添加：

```bash
export PATH="$PATH:/path/to/catch"
```

然后执行：

```bash
source ~/.bashrc  # 或 source ~/.zshrc
```

## 📖 使用说明

### 交互模式

运行 `catch` 进入交互模式：

```
┌────────────────────────────────────────────┐
│  🎯 Catch - 文件搜索工具                   │
├────────────────────────────────────────────┤
│  [1] 🔍 搜索文件内容                        │
│  [2] 📁 搜索文件名                          │
│  [3] ⚙️  高级搜索                           │
│  [4] ➕ 添加到环境变量                      │
│  [q] ❌ 退出                                │
└────────────────────────────────────────────┘

请选择功能 (1-4 或 q):
```

### 命令行模式

```bash
# 基本用法
catch search <关键字>

# 选项
catch search <关键字> [选项]

# 选项说明
-r, --recursive    递归搜索子目录
-t, --type         文件类型过滤 (如：.go,.txt)
-p, --path         搜索路径 (默认为当前目录)
```

### 搜索结果显示

- 📄 表示文件内容匹配
- 📁 表示文件名匹配
- 关键字会以黄色高亮显示

## 📁 项目结构

```
catch/
├── cmd/
│   └── catch/
│       └── main.go          # 程序入口
├── internal/
│   ├── cli/
│   │   └── cli.go           # 交互式 CLI 界面
│   └── search/
│       └── search.go        # 搜索核心逻辑
├── go.mod                   # Go 模块定义
├── README.md                # 项目说明
└── soft.txt                 # 需求文档
```

## ⚙️ 技术实现

- **协程池**: 动态缓冲区，根据协程数量自动调整 channel 缓冲大小
- **并发控制**: 使用 channel 和 sync.WaitGroup 管理并发
- **跨平台**: 纯 Go 实现，无第三方依赖（除 golang.org/x/term）
- **文件过滤**: 自动跳过 .git、node_modules 等目录
- **符号链接检测**: 避免递归遍历死循环
- **性能优化**: 使用 `strings.EqualFold` 减少内存分配，利用 `DirEntry.Info()` 避免重复 Stat 调用

## 📊 性能优化亮点

| 优化项 | 优化前 | 优化后 | 效果 |
|--------|--------|--------|------|
| 字符串比较 | `strings.ToLower` 每次分配 | `strings.EqualFold` + 缓存 | 减少 50%+ 内存分配 |
| 文件 Stat 调用 | 每个文件调用 `os.Stat` | 利用 `DirEntry.Info()` 缓存 | 减少 I/O 系统调用 |
| 缓冲区大小 | 固定 100 | 动态 `MaxGoroutine * 10` | 适配不同并发场景 |
| Scanner 缓冲 | 默认 64KB | 扩展到 1MB | 支持更长行不报错 |

## 📈 更新日志

### v1.2.0 (2026-04-05)

**新增功能**
- ✅ 正则表达式搜索（支持复杂模式匹配）
- ✅ 忽略文件支持（.catchignore / .gitignore）
- ✅ 多关键字搜索（AND/OR 逻辑组合）

**体验改进**
- ✅ 新增正则表达式搜索菜单选项
- ✅ 新增多关键字搜索菜单选项
- ✅ 高级搜索支持忽略文件配置

**技术亮点**
- ✅ 统一的匹配逻辑框架
- ✅ 支持 glob 模式的忽略文件
- ✅ 完整的测试覆盖（14+ 新测试用例）

### v1.1.0 (2026-04-04)

**新增功能**
- ✅ 实时进度显示（已扫描文件数、匹配数、扫描速度）
- ✅ 搜索耗时统计（毫秒精度）
- ✅ 路径验证（搜索前检查路径是否存在）
- ✅ 符号链接检测（避免死循环）

**性能优化**
- ✅ 字符串比较优化（减少 50%+ 内存分配）
- ✅ 文件 Stat 调用优化（利用 DirEntry 缓存信息）
- ✅ 动态缓冲区（根据协程数自动调整）
- ✅ Scanner 缓冲扩展（支持 1MB 长行）

**体验改进**
- ✅ 移除清屏闪烁（Windows cls / Unix clear）
- ✅ 优化菜单布局（适配不同终端宽度）
- ✅ 改进 Emoji 宽度计算（精确覆盖 10+ Unicode 区块）
- ✅ 协程数上限保护（最大 100 协程）

### v1.0.0 (初始版本)

- ✅ 基础搜索功能
- ✅ 交互式界面
- ✅ 协程池并发
- ✅ 关键字高亮
- ✅ 跨平台支持

## 📝 许可证

MIT License

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

## 📧 联系方式

如有问题或建议，请通过 GitHub Issues 联系。
